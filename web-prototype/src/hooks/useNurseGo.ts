...
// (Nurse GO.dc.html) para um hook React. A UI consome `vm` (view-model),
// equivalente ao antigo renderVals().
...
export function useNurseGo() {
  ...
  // ── helpers de energia ──────────────────────────────────────────
  const energiaAgora = useCallback((p: Perfil | null) => calcEnergia(p), []);
  const proximaRecarga = useCallback((p: Perfil | null) => calcRecarga(p), []);
  ...
  const vm = useMemo(() => {
    ...
    const ICONES: Record<string, string> = {
      check: "icons/list-checks.svg",
      roleta: "icons/life-buoy.svg",
      bau: "icons/gift.svg",
      geral: "icons/trophy.svg",
      caso: "icons/heart-pulse.svg",
    };
    const trilha = UNIDADES.map((u, ui) => {
      ...
      return {
        ...
        icone: ["icons/heart-pulse.svg", "icons/wind.svg", "icons/zap.svg"][ui],
        bgMapa: [
          "radial-gradient(90% 60% at 50% 0%, #EAF3FC 0%, #F5F8F9 70%)",
          "radial-gradient(90% 60% at 50% 0%, #E9F7F1 0%, #F5F8F9 70%)",
          "radial-gradient(90% 60% at 50% 0%, #FCEDF0 0%, #F5F8F9 70%)",
        ][ui],
        ...
        nos: nosU.map((n, ni) => {
          ...
          const dots =
            ni === 0
              ? []
              : [1, 2, 3].map((k) => ({
                  dx: Math.round(dxAnt + (dx - dxAnt) * (k / 4)) + "px",
          ...
          return {
            ...
            raio: n.tipo === "bau" || n.tipo === "geral" ? "20px" : "50%",
            ...
            anim: atual,
            icone: feito ? "icons/check.svg" : ICONES[n.tipo] || "",
    ...
    return {
      ...
      // tutorial
      tutBarras: TUTORIAL.map((_, i) => ({ cor: i <= (s.tutIdx || 0) ? "#05A67A" : "#DCE6EA" })),
      tutIcone: tut.icone,
      tutCor: tut.cor,
      tutBg:
      ...
      // roleta
      segmentos: SEGS.map((x, i) => ({ nome: x.nome, fg: x.fg, deg: i * 60 + 30 + "deg" })),
      wheelDeg: s.wheelDeg + "deg",
      girando: s.girando,
      sorteado: seg,
      ...
      tempoPct: pct(s.tempo, TEMPO_QUESTAO),
      tempoTxt: s.tempo + "s",
      ...
      vidasPct: pct(s.vidas || 0, (s.caso && s.caso.vidas) || 3),
      ...
      fbIcone: s.modo === "nivel" ? "icons/check.svg" : certo ? "icons/check.svg" : "icons/x.svg",
      fbTitulo:
        s.modo === "nivel"
          ? "Resposta registrada"
          : certo
      ...
      fimTexto: s.fimTexto,
      fimObito: s.fimObito,
      fimBarra: pct(s.acertos, s.fila.length),
      fimBg: s.fimObito
        ? "linear-gradient(175deg,#3A0F1B,#6E2233 60%,#8C2E3F)"
        : "linear-gradient(175deg,#0A2540,#0E3A5E 60%,#05704F)",
      acertosTxt: s.acertos + "/" + (s.fila.length || 1),
      novasConquistas: s.novas,
      confete:
        s.tela === "fim" && !s.fimObito
          ? Array.from({ length: 16 }, (_, i) => ({
              left: Math.round((i * 6.3 + (i % 3) * 4) % 96) + "%",
              cor: ["#F2C24A", "#7BE3C0", "#3FA0E8", "#F2994A", "#fff"][i % 5],
              dur: (1.5 + ((i * 7) % 9) / 10).toFixed(1) + "s",
              delay: (((i * 13) % 12) / 10).toFixed(1) + "s",
            }))
          : [],
      ...
      situacoes: SITUACOES.map((n) => ({
        nome: n,
        sel: (p.situacao || "Estudando") === n,
      })),
      ...
      temFoto: !!p.foto,
      semFoto: !p.foto,
      avPreviewBg: (AV_CORES.find((c) => c.id === (p.avCor || "azul")) || AV_CORES[0]).bg,
      avPreviewSrc: (AV_SIMBOLOS.find((x) => x.id === (p.avSimbolo || "coracao")) || AV_SIMBOLOS[0]).src,
      avCores: AV_CORES.map((c) => ({ id: c.id, bg: c.bg, sel: (p.avCor || "azul") === c.id })),
      avSimbolos: AV_SIMBOLOS.map((x) => ({ id: x.id, src: x.src, sel: (p.avSimbolo || "coracao") === x.id })),
      fotoTitulo: p.foto ? "Trocar a foto" : "Escolher uma foto",
      avatarBg: p.foto
        ? "center/cover no-repeat url(" + p.foto + ")"
        : (AV_CORES.find((c) => c.id === (p.avCor || "azul")) || AV_CORES[0]).bg,
    };
  }, [st, abrirNo, responder]);
  ...
  const acoes = useMemo(
    () => ({
      ...
      setFoto: (file: File) => {
        ...
        r.onload = () => salvar({ ...(stRef.current.perfil as Perfil), foto: String(r.result) });
      ...
      trocarNome: () => {
        const p = stRef.current.perfil as Perfil;
        const n = window.prompt("Como você quer aparecer no ranking?", p.nome || "Estudante");
        if (n && n.trim()) salvar({ ...p, nome: n.trim().slice(0, 20) });
      },
      reiniciar: () => {
        if (window.confirm("Isso apaga seu progresso e refaz o nivelamento. Continuar?")) {
          limparPerfil();
